...
_sage_const_1 = Integer(1); _sage_const_2 = Integer(2); _sage_const_10 = Integer(10); _sage_const_3 = Integer(3); _sage_const_0 = Integer(0); _sage_const_15 = Integer(15); _sage_const_0p1 = RealNumber('0.1'); _sage_const_0p3 = RealNumber('0.3'); _sage_const_5 = Integer(5); _sage_const_0p100 = RealNumber('0.100'); _sage_const_0p8 = RealNumber('0.8'); _sage_const_20 = Integer(20); _sage_const_21 = Integer(21)
...
def fac(cpr,fps):
	while True:
		...
		if c&_sage_const_3 ==_sage_const_1 :
...
def eval_candidates_for_nprimes(nprimes):
	...
	eof=False
	while not eof:
		line+=_sage_const_2 *nb_candidates
		...
		time.sleep(float(_sage_const_0p3 ))
		for i in range(nb_candidates):
		#Check only if candidate was prime
			...
			if is_L_smooth(factors[_sage_const_0 ], factors[_sage_const_2 ]):
				...
				if len(candidates)==_sage_const_5 :
					eof=True
					break
			else:
	...
	for candidate in candidates:
		flog=floor(float(log(candidate[_sage_const_1 ])/log(_sage_const_2 )))
		faclog=floor(float(log(candidate[_sage_const_0 ][-_sage_const_1 ])/log(_sage_const_2 )))
		Llog=floor(float(log(candidate[_sage_const_2 ])/log(_sage_const_2 )))

		G.write(f"{candidate[_sage_const_1 ]} is O({candidate[_sage_const_2 ]})-smooth:\n\t {candidate[_sage_const_0 ]}\n")
		G.write(f"{candidate[_sage_const_1 ]} has size 2**{flog} and 2**{faclog}-smooth with L(f,1/2) about 2**{Llog}\n\n")
	G.write(f"finished at line: {line}\n")
	
	G.close()

nprimes=_sage_const_20 

while nprimes < _sage_const_21 :
